...
export default function AdminDashboard({ darkMode }: { darkMode?: boolean }) {
  ...
  // Dark mode ke liye styling variables — AQERIONX cyan-violet theme
  const dm = darkMode;

  return (
    <div className={`flex flex-col md:flex-row min-h-screen ${dm ? "aqerionx-bg" : ""}`} style={dm ? undefined : { background: "#faf5f5" }}>
      {/* ── SIDEBAR (desktop) ── */}
      {!isMobile && (
        <aside className={`w-56 flex flex-col shrink-0 h-screen sticky top-0 ${dm ? "bg-[#0d0d25]/90 border-r border-cyan-900/30" : "bg-white border-r border-gray-200"}`}>
          <div className={`p-5 ${dm ? "border-b border-cyan-900/30" : "border-b border-gray-100"}`}>
            <div className={`text-lg font-bold ${dm ? "aqerionx-text" : "text-gray-800"}`} style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>RACE COMPUTER</div>
            <div className={`text-xs ${dm ? "text-cyan-400/60" : "text-gray-400"} mt-0.5`}>Admin Panel</div>
          </div>
            ...
            {[
              ...
              <button
                ...
                className={`w-full px-4 py-2.5 text-left text-sm flex items-center gap-3 transition-all cursor-pointer ${
                  activePanel === item.id
                    ? dm
                      ? "bg-cyan-500/10 text-cyan-400 border-l-3 border-cyan-400 font-semibold"
                      : "bg-red-50 text-red-600 border-l-3 border-red-500 font-semibold"
                    : dm
                      ? "text-gray-400 hover:bg-cyan-500/5 border-l-3 border-transparent"
          ...
          <div className={`p-4 ${dm ? "border-t border-cyan-900/30" : "border-t border-gray-100"}`}>
            {adminUser && (
              <div className="flex items-center gap-2 mb-3">
                {adminUser.photoURL ? (
                  <img src={adminUser.photoURL} alt="" className={`w-7 h-7 rounded-full border-2 ${dm ? "border-cyan-800" : "border-gray-200"}`} />
                ) : (
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-xs ${dm ? "bg-cyan-500/10 text-cyan-400" : "bg-red-50 text-red-600"}`}>{adminUser.email?.charAt(0).toUpperCase()}</div>
                )}
                <div className="min-w-0 flex-1">
                  <span className={`text-xs truncate block ${dm ? "text-gray-300" : "text-gray-700"}`}>{adminUser.email}</span>
                  {isMasterAdmin && <span className={`text-[9px] font-bold uppercase tracking-wider ${dm ? "text-cyan-400" : "text-purple-600"}`}>Master Admin</span>}
                </div>
              </div>
            )}
            <button
              onClick={adminLogout}
              className={`w-full py-2 text-sm rounded-lg transition-all cursor-pointer font-medium flex items-center justify-center gap-2 ${dm ? "text-cyan-400 border border-cyan-800 hover:bg-cyan-500/10" : "text-red-600 border border-red-200 hover:bg-red-50"}`}
            >
      ...
      {isMobile && sidebarOpen && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <aside className={`relative w-64 ${dm ? "bg-[#0d0d2b] shadow-xl shadow-cyan-900/20" : "bg-white shadow-xl"} flex flex-col h-full`}>
            <div className={`p-4 border-b ${dm ? "border-cyan-900/30" : "border-gray-100"} flex justify-between items-center`}>
              <div>
                <div className={`text-base font-bold ${dm ? "aqerionx-text" : "text-gray-800"}`}>RACE COMPUTER</div>
                <div className={`text-xs ${dm ? "text-cyan-400/60" : "text-gray-400"}`}>Admin Panel</div>
              </div>
              <button onClick={() => setSidebarOpen(false)} className={`${dm ? "text-gray-500 hover:text-cyan-400" : "text-gray-400 hover:text-gray-600"} cursor-pointer`}>
                ✕
              </button>
            </div>
                ...
                <button
                  ...
                  className={`w-full px-4 py-2.5 text-left text-sm flex items-center gap-3 transition-all cursor-pointer ${
                    activePanel === item.id
                      ...
                      : dm
                        ? "text-gray-400 hover:bg-cyan-500/5"
            ...
            <div className={`p-4 border-t ${dm ? "border-cyan-900/30" : "border-gray-100"}`}>
              <button
                onClick={adminLogout}
                className={`w-full py-2 text-sm rounded-lg transition-all cursor-pointer font-medium flex items-center justify-center gap-2 ${dm ? "text-cyan-400 border border-cyan-800 hover:bg-cyan-500/10" : "text-red-600 border border-red-200 hover:bg-red-50"}`}
              >
      ...
      <main className="flex-1 flex flex-col min-h-0">
        {/* Header */}
        <header className={`${dm ? "bg-[#0a0a1a] border-cyan-800/30" : "bg-white border-gray-200"} border-b px-4 md:px-6 py-3 flex items-center justify-between shrink-0`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className={`text-sm font-bold ${dm ? "text-cyan-300" : "text-gray-800"} flex items-center gap-2`}>
                {panelTitle}
              ...
              <p className={`text-xs ${dm ? "text-gray-500" : "text-gray-400"}`}>{panelSubtitle}</p>
            </div>
          </div>
          <a
            href="/"
            ...
            className={`text-xs ${dm ? "text-cyan-400 hover:text-cyan-300" : "text-red-600 hover:text-red-700"} font-medium flex items-center gap-1.5`}
          >
        ...
        {/* Content area */}
        <div className={`flex-1 overflow-y-auto p-4 md:p-6 ${dm ? "text-gray-100" : ""}`} style={{ paddingBottom: isMobile ? "72px" : undefined }}>
          {renderPanel()}
        </div>
      </main>
      ...
      {/* ── MOBILE BOTTOM NAV ── */}
      {isMobile && (
        <nav className={`fixed bottom-0 left-0 right-0 ${dm ? "bg-[#0a0a1a] border-cyan-800/30" : "bg-white border-gray-200"} border-t flex z-40 overflow-x-auto`} style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
            ...
            return (
              <button
                ...
                className={`flex-1 min-w-[52px] py-2.5 flex flex-col items-center gap-0.5 text-[10px] cursor-pointer transition-all relative ${
                  activePanel === item.id
                    ? dm ? "text-cyan-400 font-semibold" : "text-red-600 font-semibold"
                    : "text-gray-400"
                }`}
              >
                <span className="text-base">{item.icon}</span>
