...
export default function ServiceBooking() {
  ...
  // Firebase listener for dynamic serviceTypes
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Location not available - still submit without location
    ...
    try {
      ...
      if (firebaseError.code === "PERMISSION_DENIED") {
        toast.error("Submission blocked", { description: "Firebase rules need to be updated. Please contact admin." });
      } else {
        toast.error("Failed to submit", { description: "Please try again or call us directly." });
      }
    }
    setLoading(false);
  };
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center">
      ...
      <div ref={modalRef} className="relative bg-white/95 backdrop-blur-2xl rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-lg max-h-[92vh] sm:max-h-[90vh] overflow-y-auto border border-white/30">
        {/* Header */}
        ...
        <div className="p-4 sm:p-5">
          {submitted ? (
            <div className="text-center py-6 sm:py-8 space-y-3 sm:space-y-4">
              ...
              <Button onClick={() => setBookingModalOpen(false)} className="bg-gradient-to-r from-blue-600 to-red-500 text-white text-sm">
                Done
              </Button>
            </div>
          ) : (
